...
const GLOBAL_WINDOW_POLICY =
  'MANDATORY WINDOW POLICY: start each automation flow by creating a NEW browser window via chrome_navigate with newWindow=true.';
const EXPLICIT_TARGET_POLICY =
  'After the new window is created, every subsequent browser tool call MUST explicitly include BOTH windowId and tabId. Never rely on active-tab defaults.';
const NAVIGATE_POLICY =
  'For initialization calls, chrome_navigate MUST set newWindow=true. When continuing in an existing context, still pass explicit windowId/tabId.';

function withToolUsagePolicy(tool: Tool): Tool {
  const baseDescription = (tool.description ?? '').trim();
  const isNavigate = tool.name === TOOL_NAMES.BROWSER.NAVIGATE;
  const policyText = isNavigate
    ? `${GLOBAL_WINDOW_POLICY} ${NAVIGATE_POLICY}`
    : `${GLOBAL_WINDOW_POLICY} ${EXPLICIT_TARGET_POLICY}`;
...
export const TOOL_SCHEMAS: Tool[] = BASE_TOOL_SCHEMAS.map(withToolUsagePolicy);
